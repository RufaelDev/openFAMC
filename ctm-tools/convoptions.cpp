...
Options::Options()
{
  ...
  mComment = string("");
  mTexFileName = string("");
  mFamcParamFile= string("famcParamFile.txt");
  mExportExtension = string(".off");
}
...
void Options::GetFromArgs(int argc, char **argv, int aStartIdx)
{
  for(int i = aStartIdx; i < argc; ++ i)
  {
	...
	else if(cmd == string("--Exp"))
    {
       mExportExtension = string(argv[i + 1]);
	   i++;
    }
	...
	else if(cmd == string("--famcParamFile"))
    {
      mFamcParamFile = string(argv[i + 1]);
	  ++i;
    }
    else
